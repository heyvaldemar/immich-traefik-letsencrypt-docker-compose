#!/bin/bash
# End-to-end tests for the immich-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-immich}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-immich-traefik-letsencrypt-docker-compose.yml}"
...
: "${POSTGRES_BACKUPS_PATH:=/srv/immich-postgres/backups}"
: "${POSTGRES_BACKUP_NAME:=immich-postgres-backup}"
: "${BACKUP_INTERVAL:=24h}"
: "${IMMICH_DB_NAME:=immich}"
: "${IMMICH_DB_USER:=immichdbuser}"
: "${DATA_BACKUPS_PATH:=/srv/immich-library/backups}"
: "${DATA_BACKUP_NAME:=immich-library-backup}"
