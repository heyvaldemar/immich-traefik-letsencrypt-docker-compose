...
#
# Immich's database is not stock PostgreSQL - it carries VectorChord and
# pgvector, and the server checks the VectorChord version at startup. A plain
# pg_dump carries both extensions across, because it writes them as CREATE
# EXTENSION statements rather than as data; the new image supplies them at the
# versions Immich supports.
#
...
COMPOSE_FILE="immich-traefik-letsencrypt-docker-compose.yml"
...
dump="immich-postgres-${running}-to-${pinned}-${stamp}.sql.gz"
