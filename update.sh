...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/immich-traefik-letsencrypt-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
...
# Immich ever connects. Read both majors before the checkout, while the old
